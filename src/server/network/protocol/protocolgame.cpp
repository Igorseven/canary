...
void ProtocolGame::addImbuementInfo(NetworkMessage &msg, uint16_t imbuementId, bool isScroll) const {
	...
	const uint32_t storage = imbuement->getStorage();
	if (g_configManager().getBoolean(TOGGLE_IMBUEMENT_SHRINE_STORAGE) && storage != 0 && player->getStorageValue(storage) != -1) {
		msg.addByte(0x01);
	} else {
		msg.addByte(0x00);
	}
...
void ProtocolGame::openImbuementWindow(const Imbuement_Window_t type, const std::shared_ptr<Item> &item /*= nullptr*/) {
	...
	if (item == nullptr || type == IMBUEMENT_WINDOW_CHOICE) {
		...
		const uint32_t money = player->getMoney() + player->getBankBalance();
...
void ProtocolGame::parseSelectSpellAimProtocol(NetworkMessage &msg) {
	...
	player->setNextExAction(OTSYS_TIME() + g_configManager().getNumber(EX_ACTIONS_DELAY_INTERVAL) - 10);
}
